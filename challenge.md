...
but your final code should reflect *your* work.  Please upload all project files
(not your `venv` directory) directly into Canvas before the deadline.
...
**Keep in mind, the packages above are all you will need to install in order to complete
the Sprint Challenge. Additional packages may not be recognized by the container running
in CodeGrade and cause your auto-test files to fail.  Please know we will be checking each
Sprint Challenge individually for correctness/completeness.**
...
It is also *optional* to manage your virtual environment - it's highly
suggested if available, but `pip install Flask Flask-SQLAlchemy requests` is
adequate for passing.

This file is Markdown, so it may be helpful to open with PyCharm to view it nicely rendered.
...
you may `pip install py-openaq` if you wish, but do note that it has
a number of dependencies you'll also have to have in your environment (`pandas`,
`seaborn`, `matplotlib`, `requests`) which most likely won't let the auto tests run in CodeGrade.
We really only care about that last one - [requests](https://pypi.org/project/requests/)
- as it is what gets the data over the network (the other dependencies allow the API object
to return more convenient things like dataframes and plots).

So, you have also been provided with a file `openaq.py` that provides a minimal
wrapper, only depending on `requests`. You'll *still* need to `pip`
...
To test and verify, you can execute`FLASK_APP=aq_dashboard.py flask shell` - Flask
shell will open a Python REPL in the context of your application:
...
get few or possibly even no values above the threshold. You can double-check by
...
_**Please also add a screenshot of your running application loaded locally in a web
browser, to facilitate grading and feedback**_. Thanks for your hard work!
